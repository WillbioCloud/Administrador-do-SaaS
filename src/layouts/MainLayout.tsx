...
const navigation = [
  {
    ...
    items: [
      ...
      { name: "Planos", href: "/plans", icon: Briefcase },
    ]
  },
  {
    title: "Financeiro",
...
export default function MainLayout() {
  ...
  const getBreadcrumbs = () => {
    ...
    if (path === "/clients") return ["Dashboard", "Gestão Principal", "Clientes"]
    if (path === "/plans") return ["Dashboard", "Gestão Principal", "Planos"]
    if (path === "/payments") return ["Dashboard", "Financeiro", "Pagamentos"]
    if (path === "/contracts") return ["Dashboard", "Financeiro", "Contratos"]
    if (path === "/settings") return ["Dashboard", "Sistema", "Definições"]
    if (path === "/support") return ["Dashboard", "Sistema", "Suporte"]
    return ["Dashboard"]
  }
    ...
    <div className="h-screen w-full flex overflow-hidden bg-[#f8f9fa] dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans transition-colors duration-200">
        ...
        <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center gap-2">
                ...
                {breadcrumbs.map((crumb, idx) => (
                  <React.Fragment key={crumb}>
                    <span className={idx === breadcrumbs.length - 1 ? "text-slate-900 dark:text-slate-100 font-medium" : ""}>
                      {crumb}
                    </span>
          ...
          <div className="flex items-center gap-x-4">
            ...
            <Button 
              variant="outline" 
              size="sm" 
              className="hidden sm:flex h-9 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
            ...
            <div className="flex items-center gap-1 border border-slate-200 dark:border-slate-700 rounded-md p-0.5 bg-slate-50 dark:bg-slate-800">
              <Button variant="ghost" size="icon" className={cn("h-7 w-7 rounded-sm", !isDark ? "bg-white shadow-sm text-slate-900" : "text-slate-400 hover:text-slate-300")} onClick={() => setIsDark(false)}>
                <Sun className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" className={cn("h-7 w-7 rounded-sm", isDark ? "bg-slate-700 shadow-sm text-white" : "text-slate-400 hover:text-slate-600")} onClick={() => setIsDark(true)}>
            ...
            <div className="relative" ref={notificationsRef}>
              <Button 
                ...
                size="icon" 
                className={cn(
                  "relative h-9 w-9 border border-slate-200 dark:border-slate-700 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800",
      ...
      {isNewTaskOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                <Plus className="h-4 w-4 text-indigo-500" />
                Criar Nova Tarefa
              </h3>
              <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300" onClick={() => setIsNewTaskOpen(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Título da Tarefa</label>
                <Input placeholder="Ex: Ligar para o cliente X" className="bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 dark:text-white" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Descrição (Opcional)</label>
                <textarea 
                  className="w-full min-h-[80px] rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 dark:focus-visible:ring-slate-300 resize-none"
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
              <Button variant="outline" className="bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300" onClick={() => setIsNewTaskOpen(false)}>Cancelar</Button>
              <Button className="bg-indigo-600 text-white hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600" onClick={() => {
